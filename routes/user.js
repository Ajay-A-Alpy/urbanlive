...
router.get("/", async function (req, res, next) {
  ...
  if (userLogged) {
    ...
    let expirycheck = await userHelpers
    ...
    coupenValid = await userHelpers
      .myCoupenValidity(req.session.userId, currentTime)
      .catch(() => {
    ...
    referValid = await userHelpers
      .myReferalCheck(req.session.userId)
      .catch((err) => {
        console.log(err);
      });
...
router.get("/show-category/:id", async function (req, res) {
  ...
  if (pro) {
    ...
    res.render("user/view-categoryPro", {
      message,
      pots,
      Plants,
      user,
...
router.get("/login", function (req, res) {
  ...
  if (userLogged) {
    ...
    res.render("user/login", {
      ...
      changePassword: req.session.passwordMessage,
    });
...
router.post("/signup", async function (req, res) {
  ...
  userHelpers
    .existMailChecker(req.body.email)
    .then(() => {
...
router.post("/signup-otp", (req, res) => {
  ...
  client.verify
    ...
    .then((response) => {
      if (response.valid) {
        userHelpers
          .doSignUp(req.session.newSignup, req.session.referal)
          .then((data) => {
...
router.post("/getotp", function (req, res) {
  ...
  client.verify
    ...
    .then((response) => {
      if (response.valid) {
        userHelpers
          .getUserdetailsWithMobile(req.session.mobileNumber)
          .then((result) => {
...
router.post("/change-password", (req, res) => {
  if (req.session.loggedIn) {
    ...
    userHelpers
      .passwordUpdate(req.session.mobileNumber, req.body.password)
      .then(() => {
...
router.post("/place-order", async (req, res) => {
  ...
  userHelpers
    .placeOrder(user, req.body, products, req.session.orderAmt)
    .then((orderId) => {
      ...
      if (req.body["payment-method"] === "COD") {
        ...
        userHelpers
          .generateRazorPay(orderId, req.session.orderAmt)
          .then((response) => {
            res.json(response);
          });
      }
...
router.post("/verify-payment", verifyLogin, async function (req, res) {
  ...
  userHelpers
    .verifyPayment(req.session.orderId, req.body)
    .then(() => {
      userHelpers
